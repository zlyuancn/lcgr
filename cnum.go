...
const primeNumber1e10 uint64 = 9999999967 // 一百亿以内中最大的一个质数
...
func Confuse(sn, seed uint64) uint64 {
	...
	for i := uint64(0); i < 5; i++ { // ps: 在不交换的情况下, 经过实际验证, 重复5次混淆低位值会出现视觉上的随机性
		// (snH+i)%mod or (snL+i)%mod 使用 i 对混淆sn值做基础偏移, 不同的sn必然得到不同的值.
		vH = confuse((snH+i)%mod, offset) % mod // 高位混淆. ps: 相同的高位其结果必然相同, 所以 vH 必然是同一个值

...
由于每次生成id, 需要先去redis incr获取一个新的序号做映射, 在高并发时对同一个key造成压力.

可以使用多个分片key进行 incr , 比如1000个, 此时有如下方式将其组合

1. **(推荐)** 结果为 `映射(序号id)_分片id`
...
考虑每秒生成 `1亿` 个数, 需要 317 年 (`1e18/1e8/86400/365`) 才消耗完毕. 完全无需担心.
